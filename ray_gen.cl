    ...
    )
{
	...
	dir = normalize(dir);
	ray_data[(x + width * y) * ray_size] = cameraposx;
    ray_data[(x + width * y) * ray_size + 1] = cameraposy;
    ray_data[(x + width * y) * ray_size + 2] = cameraposz;
    ray_data[(x + width * y) * ray_size + 4] = dir.x;
    ray_data[(x + width * y) * ray_size + 5] = dir.y;
    ray_data[(x + width * y) * ray_size + 6] = dir.z;
