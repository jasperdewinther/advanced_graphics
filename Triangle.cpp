...
std::vector<Triangle> get_mesh_from_file(const std::string& filename, float scalefactor, float3 position, Material material){
    ...
    for (int i = 0; i < lines.size(); i++) { //append all vertices
        if (lines[i][0] == 'f' && lines[i][1] == ' ') {
            ...
            float3 p0 = vertices[std::stoi(part0[0])-1]*scalefactor+ position;
            float3 p1 = vertices[std::stoi(part1[0])-1]*scalefactor+ position;
            float3 p2 = vertices[std::stoi(part2[0])-1]*scalefactor+ position;

            float3 n0 = normals[std::stoi(part0[2])-1];
            float3 n1 = normals[std::stoi(part1[2])-1];
            float3 n2 = normals[std::stoi(part2[2])-1];
            float3 n = normalize(float3((n0.x + n1.x + n2.x)/3.f, (n0.y + n1.y + n2.y) / 3.f, (n0.z + n1.z + n2.z) / 3.f));
