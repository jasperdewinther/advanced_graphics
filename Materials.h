...
struct MaterialData
{
	...
	float transparent; // 0 if not transparent, 1 if completely transparent
	float refractive_index; 
	MaterialData(std::variant<float3, std::function<float3(float3, float3)>> color, float specularity, float transparent=0, float refractive_index=0);
...
const MaterialData materials[9] = {
	...
	MaterialData(float3(1.f,1.f,1.f), 0.0f, 1.0, 1.52),
	MaterialData(float3(1.f,0.f,0.f), 0.0f, 0.8, 1.52),
};